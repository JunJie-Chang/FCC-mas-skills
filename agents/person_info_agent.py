...
7. 引用規則：每當文中出現具體數字（金額、百分比、排名、資本額、人數等統計數字），在該數字後標 [N]（N 從 1 開始的整數），並在 JSON 最外層加入 "references" 陣列，每條格式為 {{"num": N, "title": "來源標題", "url": "來源網址"}}。URL 必須取自上方搜尋資料「來源」欄位的原始網址，不可捏造。若某數字無法確認來源，不標引用號。
{mode_rules}
