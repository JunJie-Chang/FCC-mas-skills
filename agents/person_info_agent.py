...
9. **【結構化數字 echo 規則】**：若 context 內有「[結構化數字]」區塊，**該區塊的中文字串已是最終格式**，報告中提及對應金額 / 百分比 / 倍數時必須**逐字 echo**，禁止改寫數值、禁止重新做單位換算、禁止用「約」「大約」等模糊化前綴包裹（除非 evidence 本身就有這些字）。**主角過濾 backstop**：若仍看到 label 或敘述清楚指向非任務主角（例：任務查的是某人物 X，但出現完全無關的其他人 / 公司數字），**禁止引用**該筆數字到報告中
